...
class TestQuizesRouter:
    def test_get_quizes(self, client, seed_quizes_to_you):
        """Test the "/my/quizzes" endpoint."""

        res = client.get("/my/quizzes")
    ...
    def test_create_quiz(self, client, seed_quizes_to_you, quiz_2):
        """Test the "/my/quizzes" POST endpoint."""

        with test_db_session() as db:
            num_quizes_before = db.query(Quiz).count()

        res = client.post("/my/quizzes", json=quiz_2)
    ...
    def test_get_one_quiz(self, client, quiz_1, seed_you):
        """Test the "/my/quizzes/{quiz_id}" endpoint."""
        ...
        res = client.get(f"/my/quizzes/{quiz_id}")
        res_json = res.json()

        assert res.status_code == status.HTTP_200_OK
        assert quiz_1["name"] == res_json["name"]

        res = client.get("/my/quizzes/10000")

        assert res.status_code == status.HTTP_404_NOT_FOUND

    def test_update_quiz(self, client, seed_quizes_to_you, quiz_2):
        """Test the "/my/quizzes/{quiz_id}" PUT endpoint."""

        with test_db_session() as db:
            quiz_before = db.query(Quiz).first()

        res = client.put(f"/my/quizzes/{quiz_before.id}", json=quiz_2)
        ...
        res = client.put(f"/my/quizzes/10000", json=quiz_2)

        assert res.status_code == status.HTTP_404_NOT_FOUND

    def test_delete_quiz(self, client, seed_quizes_to_you):
        """Test the "/my/quizzes/{quiz_id}" DELETE endpoint."""

        with test_db_session() as db:
            quiz_before = db.query(Quiz).first()
            num_quiz_before = db.query(Quiz).count()

        res = client.delete(f"/my/quizzes/{quiz_before.id}")

        assert res.status_code == status.HTTP_200_OK
        with test_db_session() as db:
            assert db.query(Quiz).count() == num_quiz_before - 1

        res = client.delete(f"/my/quizzes/10000")

        assert res.status_code == status.HTTP_404_NOT_FOUND

    def test_get_one_quizes_questions(self, client, seed_quizes_to_you):
        """Test the "/my/quizzes/{quiz_id}/questions" endpoint."""

        with test_db_session() as db:
            quiz_db = db.query(Quiz).first()
            questions = quiz_db.questions

        res = client.get(f"/my/quizzes/{quiz_db.id}/questions")
        res_json = res.json()
        ...
        res = client.get("/my/quizzes/10000/questions")

        assert res.status_code == status.HTTP_404_NOT_FOUND

    def test_add_question_to_quiz(self, client, seed_quizes_to_you, question_3):
        """Test the "/my/quizzes/{quiz_id}/add-question" POST endpoint."""
        ...
        res = client.post(
            f"/my/quizzes/{quiz_db.id}/add-question", json={"question_id": question_id}
        )
        ...
        res = client.post(
            "/my/quizzes/10000/add-question", json={"question_id": question_id}
        )
